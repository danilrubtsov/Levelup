...
class FoodList {
 ...
 public:
  ...
  void add(FoodItem item) {
    ...
    // как в стек
    e->next = root;//root = NULL - признак корня
    root = e;
  }
...
int main() {
  ...
  FoodList foodList;//объявили экземпляр класса FoodList
  assert(foodList.isEmpty());

  FoodItem milk;//объявили экземпляр класса FoodItem
  milk.name = "Молоко";
  milk.amount = 1;
  milk.measureUnit = "л";
  ...
  m2.amount = 400;
  m2.measureUnit = "г.";// Единица измерения
  foodList.add(m2);
